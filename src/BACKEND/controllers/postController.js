...
exports.post_detail = async (req, res, next) => {
  ...
  if (!post) return res.status(404).json({ err: 'genre with the given id is not found' })
  // if (post === null) {
...
exports.post_create_post = [
  ...
  body('title').trim().isLength({ min: 3, max: 200 }).withMessage('Title content must between 3 & 200 charts.').escape(),
  ...
  async (req, res, next) => {
    ...
    if (errors.array().length > 0) {
      ...
      res.json({ post , postUrl: post.url })
    }
  },
]
